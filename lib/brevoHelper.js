...
const handleNewRecruitment = async (teacherEmail, teacherName, companyName, jobTitle, salary, location, employerMessage, jobId) => {
    const apiKey = process.env.BREVO_API_KEY;
        ...
        sender: {
          ...
          <html>
           <body>
            ...
            <br/>
            <br/>
            View the link here: <a href="https://ajarnjobs.com/jobs/${jobId}/${jobTitle.replace(/\s+/g, "-")}">${jobTitle}</a>
            <br/>
            <br/>
            <p>
              Best regards,
...
const handleAdminStripeTransactionNotification = async (totalAmount, transaction) => {
  const apiKey = process.env.BREVO_API_KEY;
...
const handleAdminJobReportNotification = async (userName, userEmail, job, feedbackType, feeback) => {
  const apiKey = process.env.BREVO_API_KEY;
  ...
  const payload = {
    ...
    subject: `A Job Violation Report has been submitted by ${userName}.`,
    htmlContent: `
    <html>
     <body>
      <p>Heads up! ${userName} has submitted a job violation report for ${job.title}.</p>
      ...
      <p>Job Reason Type: ${feedbackType}</p>
      <p>Job Reason: ${feeback}</p>
      <br />
      View the link here: <a href="https://ajarnjobs.com/jobs/${job._id}/${job.title.replace(/\s+/g, "-")}">${job.title}</a>
      <br/>
      <p>Best regards,</p>
      <p>The Ajarn Jobs Team</p>
      <br/>
...
const handleNewMessageEmailNotification = async (senderName, recieverEmail, message) => {
  const apiKey = process.env.BREVO_API_KEY;
    ...
    const payload = {
        ...
        <html>
         <body>
          <h4>Message from:</h4>
          <p><b>${senderName}</b>: ${message}</p>
          <br/>
          <p>To view the message, please login to your account, head to your dashboard and view your latest messages.</p>
          <br/>
          <p>Best regards,</p>
          <p>The Ajarn Jobs Team</p>
          <br/>
...
module.exports = {
    ...
    handleNewMessageEmailNotification,
    handleAdminJobReportNotification
}